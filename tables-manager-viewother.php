...
<table width="100%" border="0" cellspacing="0" cellpadding="2" style="font-weight:bold;font-size: 12px;">
	...
	?>	
		<td style="background:#DBEAD3; text-align:center;">
			<select class="agentchange<?php echo $ID ?>" name="agentchange" onchange="selectagent(this,<?php echo $ID ?>)"><option value=""><?php echo $agentname ?></option><optgroup label="-------"></optgroup><?php echo $agentsdropbox ?>
		</td>
		...
		<td style="background:#DBEAD3; text-align:center;width:80px">
			<!--<select class="statuschange<?php echo $ID ?>" name="statuschange" onchange="checkselect(this.selectedIndex,<?php echo $ID ?>)">
			<option value=""><?php echo $Status ?></option><optgroup label="-------"></optgroup><option value="Quote Sent">Quote Sent</option><option value="Offer Reminder">Offer Reminder</option><option value="Booked">Booked</option><option value="Cancelled">Cancelled</option><option value="Duplicate">Duplicate</option></select>-->
		<?php echo $Status ?>
		</td>
			<td style="width:20px; padding:0px;background:#DBEAD3;"><button href="#" class="toggler" data-prod-cat="<?php echo $ID ?>"><img id="statusimg<?php echo $ID ?>" style="margin:2px;" src="images/edit.png" /></button></td>
...
<h5 style="font-size:11px;font-weight:bold;margin: 0 0 5px 7px; color:red;">please save each change before entering other changes</h5>
...
<?php
$idrequest=$ID;
$itinerarydata = mysqli_query($dbconn, "SELECT * FROM itineraries WHERE request_id='$idrequest'");
mysqli_query($dbconn, "SET NAMES UTF8");
